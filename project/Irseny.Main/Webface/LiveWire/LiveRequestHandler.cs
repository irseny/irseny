	...
	public class LiveRequestHandler {
		...
		public JsonString Answer(JsonString request, int origin) {
			...
			do {
				...
				answer.AddTerminal("type", StringifyTools.StringifyString(type));
				...
				if (requestId > -1) {
					answer.AddTerminal("requestId", StringifyTools.StringifyInt(requestId));
				}
				...
				if (topic.Equals("camera")) {
					AnswerCameraRequest(subject, type, answer);
				} else if (topic.Equals("origin")) {
					AnswerOriginRequest(subject, type, origin, answer);
				} else if (topic.Equals(string.Empty)) {
					status = HttpStatusCode.BadRequest;
					break;
				}
			} while (false);
		...
		private void CreateErrorAnswer(JsonString answer, HttpStatusCode status) {
			switch (answer.Type) {
			case JsonStringType.Dict:
				answer.AddTerminal("status", StringifyTools.StringifyInt((int)status), true);
				break;
			default:
				// TODO pass client info to generate a sensible answer
				answer.AddTerminal("status", StringifyTools.StringifyInt((int)status));
				break;
			}
		}
	}
